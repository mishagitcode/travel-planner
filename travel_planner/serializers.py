...
class TravelProjectCreateSerializer(serializers.ModelSerializer):
    place_ids = PlaceExternalIdSerializer(many=True, write_only=True, required=False, default=list)
    ...
    def validate_place_ids(self, value):
        ...
        for item in value:
            ...
            if place_data is None:
                raise serializers.ValidationError(f'Place "{ext_id}" not found in the external API.')
    ...
    def create(self, validated_data):
        ...
        ProjectPlace.objects.bulk_create([
            ProjectPlace(project=project, **place)
